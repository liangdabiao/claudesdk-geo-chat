...
[UPLOAD_DIR, REPORT_DIR].forEach((d) => { if (!fs.existsSync(d))
...
wss.on("connection", (ws) => {
    ...
    ws.on("close", () => { for (const session of sessions.values())
        session.subscribers.delete(ws); });
});
setInterval(() => { wss.clients.forEach((ws) => { const c = ws; if (!c.isAlive)
    return c.terminate(); c.isAlive = false; c.ping(); }); }, 30000);
